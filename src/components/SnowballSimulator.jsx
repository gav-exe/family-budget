...
const TEXT_COLORS = ['text-cyan-400', 'text-blue-400', 'text-purple-400', 'text-amber-400', 'text-red-400', 'text-pink-400', 'text-green-400'];
const BG_COLORS = ['bg-cyan-500/10', 'bg-blue-500/10', 'bg-purple-500/10', 'bg-amber-500/10', 'bg-red-500/10', 'bg-pink-500/10', 'bg-green-500/10'];
const BORDER_COLORS = ['border-cyan-500/30', 'border-blue-500/30', 'border-purple-500/30', 'border-amber-500/30', 'border-red-500/30', 'border-pink-500/30', 'border-green-500/30'];
  ...
  if (!simulation) {
    return (
      <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-8 text-center">
        ...
        <h3 className="text-xl font-bold text-green-400">You're debt free!</h3>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="bg-gradient-to-r from-cyan-500/10 to-green-500/10 rounded-xl p-6 border border-cyan-500/20">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-white">Snowball Payoff Plan</h2>
            <p className="text-sm text-slate-400">Attack the smallest balance first, roll payments into the next</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-800/60 rounded-lg p-4">
            ...
            <div className="text-lg font-bold text-cyan-400">{fmt(monthlyBudget)}</div>
            <div className="text-[10px] text-slate-500">minimums + {fmt(debtAttack)} extra</div>
          </div>
          <div className="bg-slate-800/60 rounded-lg p-4">
            <div className="flex items-center gap-1.5 text-xs text-slate-500 mb-1">
          ...
          <div className="bg-slate-800/60 rounded-lg p-4">
            ...
            <div className="text-lg font-bold text-green-400">{timeStr}</div>
          </div>
          <div className="bg-slate-800/60 rounded-lg p-4">
            <div className="flex items-center gap-1.5 text-xs text-slate-500 mb-1">
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 p-5">
        ...
        <p className="text-sm text-slate-400 mb-5">Each debt falls off, freeing up more money for the next one.</p>
              ...
              return (
                <div key={i} className="flex flex-col items-center gap-1" style={{ minWidth: totalMonths > 30 ? 14 : 20 }}>
                  ...
                  {(snap.month % (totalMonths > 24 ? 6 : 3) === 0 || snap.month === totalMonths) && (
                    <span className="text-[9px] text-slate-500 -rotate-45 origin-top-left whitespace-nowrap mt-1">
                      {snap.month}mo
                    </span>
                  )}
                </div>
      ...
      <div className="bg-slate-800 rounded-xl border border-slate-700 p-5">
        ...
        <div className="space-y-2 text-sm text-slate-400">
          <p>
            <span className="text-white font-medium">Total monthly budget:</span> {fmt(monthlyBudget)}
            ({fmt(simulation.totalMinPayments)} in minimums + {fmt(simulation.debtAttack)} debt attack from both incomes)
          </p>
          <p>
            <span className="text-white font-medium">Strategy:</span> Pay minimums on everything, throw all extra at
            <span className="text-cyan-400 font-medium"> {activeDebts[0]?.name}</span> first (smallest balance).
          </p>
          <p>
