...
export default function Subscriptions() {
  ...
  function SubRow({ sub }) {
    return (
      <tr className="border-t border-slate-700/50 hover:bg-slate-700/30 group">
        <td className="px-5 py-3">
          <EditableCell value={sub.name} type="text" onChange={v => dispatch({ type: 'UPDATE_SUBSCRIPTION', id: sub.id, field: 'name', value: v })} className="text-slate-300" />
        </td>
        ...
        <td className="px-5 py-3 text-center">
          <button
            ...
            className={`text-xs px-2.5 py-1 rounded-full font-medium cursor-pointer transition-colors ${sub.status === 'Active' ? 'bg-green-500/20 text-green-400 hover:bg-green-500/30' : 'bg-slate-600/30 text-slate-500 hover:bg-slate-600/50'}`}
          >
            {sub.status}
          </button>
        </td>
        <td className="px-5 py-3">
          <EditableCell value={sub.notes || ''} type="text" onChange={v => dispatch({ type: 'UPDATE_SUBSCRIPTION', id: sub.id, field: 'notes', value: v })} className="text-slate-500 text-sm" />
        </td>
        <td className="px-2 py-3">
          <button onClick={() => dispatch({ type: 'REMOVE_SUBSCRIPTION', id: sub.id })} className="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-300 transition-all cursor-pointer">
            <Trash2 size={14} />
          </button>
        </td>
      </tr>
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-800 rounded-xl p-5 border border-slate-700">
          ...
          <div className="text-2xl font-bold text-amber-400">{fmt(totalCost)}</div>
        </div>
        <div className="bg-slate-800 rounded-xl p-5 border border-slate-700">
        ...
        <div className="bg-slate-800 rounded-xl p-5 border border-slate-700">
          <span className="text-sm text-slate-400">Yearly Cost</span>
          <div className="text-2xl font-bold text-red-400">{fmt(totalCost * 12)}</div>
        </div>
      </div>
        ...
        <table className="w-full">
          ...
          <tfoot>
            <tr className="border-t border-slate-700 bg-slate-700/30 font-semibold">
              ...
              <td className="px-5 py-3 text-right text-amber-400">{fmt(totalCost)}</td>
              <td></td>
              <td></td>
              <td></td>
            </tr>
