    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-800 rounded-xl p-5 border border-slate-700">
          ...
          {debtChartData.length > 0 ? (
            ...
            <div className="flex items-center justify-center h-[200px] text-green-400 font-semibold">All debts paid off!</div>
          )}
      ...
      {snowballOrder.length > 0 && (
        <div className="bg-slate-800 rounded-xl p-5 border border-slate-700">
          ...
          <p className="text-sm text-slate-400 mb-4">Pay minimums on everything, throw extra at the smallest balance first.</p>
            ...
            {snowballOrder.map((d, i) => {
              ...
              return (
                <div key={d.id} className="flex items-center gap-4">
                  <span className={`w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${i === 0 ? 'bg-cyan-500/20 text-cyan-400 ring-2 ring-cyan-500' : 'bg-slate-700 text-slate-400'}`}>
                    {i + 1}
                  </span>
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-1">
                      <span className={`text-sm font-medium truncate ${i === 0 ? 'text-cyan-400' : 'text-slate-300'}`}>{d.name}</span>
                      <span className="text-sm text-slate-400">{fmt(d.currentBalance)}</span>
                    </div>
                    <ProgressBar value={d.originalBalance - d.currentBalance} max={d.originalBalance} color={i === 0 ? 'bg-cyan-500' : 'bg-slate-500'} />
                  </div>
                  <span className="text-xs text-slate-500 shrink-0 w-16 text-right">~{monthsLeft} mo</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="bg-slate-800 rounded-xl p-5 border border-slate-700">
        <div className="flex justify-between items-center">
          ...
          <span className="text-amber-400 font-bold">{fmt(subTotal)}/mo</span>
        </div>
      </div>
    </div>
  );
